...
class CustomSubtitleContainer {
  ...
  private async requestStateWithRetry(attempt: number = 0): Promise<void> {
    try {
      ...
      this.isEnabled = result.extensionEnabled && !result.useAutoGeneratedSubtitles
      ...
      if (this.isEnabled) {
        console.log("[Custom Subtitles] Custom subtitle mode enabled, initializing")
        this.init()
      } else {
        console.log("[Custom Subtitles] Custom subtitle mode disabled")
      }
    } catch (error) {
      console.error(`[Custom Subtitles] Error getting state (attempt ${attempt + 1}/${this.maxInitAttempts}):`, error)
      ...
      if (attempt < this.maxInitAttempts - 1) {
        ...
        console.error("[Custom Subtitles] Failed to get state after all retries, assuming disabled")
                ...
                (subResponse) => {
                  if (chrome.runtime.lastError) {
                    ...
                    resolve({
                      ...
                      useAutoGeneratedSubtitles: subResponse?.useAutoGenerated || false
                    })
                  }
                }
              )
  ...
  private init(): void {
    if (!this.isEnabled) {
      console.log("[Custom Subtitles] Extension disabled, skipping initialization")
      return
    }
  ...
  public setEnabled(enabled: boolean): void {
    ...
    console.log(`[Custom Subtitles] Extension ${enabled ? "enabled" : "disabled"}`)
  ...
  private renderWordCard(): void {
    ...
    const containerRect = this.subtitleContainer?.getBoundingClientRect() || null
  ...
  private updateSubtitles(): void {
    if (!this.videoElement || !this.subtitle1Element || !this.subtitle2Element || !this.isEnabled) return
  ...
  private processSubtitleElement(element: HTMLDivElement, text: string): void {
    if (!this.isJapaneseEnabled || !this.isInitialized || !this.isJapaneseText(text)) {
    ...
    element.addEventListener("mouseenter", this.handleSubtitleMouseEnter.bind(this))
    element.addEventListener("mouseleave", this.handleSubtitleMouseLeave.bind(this))
  }
  ...
  public async loadSubtitleFromUrl(url: string, trackNumber: 1 | 2): Promise<void> {
    if (!this.isEnabled) {
      console.log("[Custom Subtitles] Extension disabled, not loading subtitles")
      return
    }
  ...
  private setupMessageListener(): void {
    ...
    window.addEventListener("persistSubtitleSelection", (event: CustomEvent) => {
      if (!this.isEnabled) return
