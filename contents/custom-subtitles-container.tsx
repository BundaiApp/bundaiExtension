...
class CustomSubtitleContainer {
      ...
      chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
        ...
        if (message.action === "setUseAutoGeneratedSubtitles") {
          console.log(
            ...
            message.useAutoGenerated
          )
          if (this.isEnabled && message.useAutoGenerated) {
            this.fetchAutoGeneratedSubtitles()
          }
          sendResponse({ success: true })
          return true
        }

        // New 3-state mode handler
        if (message.action === "setSubtitleMode") {
          const mode = message.subtitleMode as "api" | "auto" | "user"
          console.log("[Custom Subtitles] setSubtitleMode:", mode)
          
          // Ensure container is enabled and set up
          if (!this.isEnabled) {
          ...
          if (mode === "api") {
            // Load saved API subtitles
            this.loadSavedSubtitles()
          } else if (mode === "user") {
            // User mode - wait for upload, or load if already stored
            const videoId = this.extractVideoId(window.location.href)
            if (videoId) {
              this.loadSavedUserSubtitles(videoId)
            }
          } else if (mode === "auto") {
            // Fetch auto-generated
            this.fetchAutoGeneratedSubtitles()
          }
        ...
        // Handler for user-uploaded subtitles
        ...
        // Handler to clear user subtitles
  ...
  // Load user-uploaded subtitles from storage
  private async loadSavedUserSubtitles(videoId: string): Promise<void> {
    try {
      ...
      if (saved?.cues) {
        ...
        this.lastProcessedSubtitle1Text = ""
        console.log(`[Custom Subtitles] Loaded user subtitle: ${saved.fileName} (${adjustedCues.length} cues)`)
      } else {
        console.log("[Custom Subtitles] No user subtitle found for this video")
      }
    } catch (error) {
      console.error("[Custom Subtitles] Failed to load user subtitle:", error)
    }
  }

  private async fetchAutoGeneratedSubtitles(): Promise<void> {
    if (!this.videoElement) return

    const videoId = this.extractVideoId(window.location.href)
    if (!videoId) {
      console.log(
        "[Custom Subtitles] No video ID found for auto-generated fetch"
      )
      return
    }
    ...
    try {
      console.log(
        "[Custom Subtitles] Fetching auto-generated subtitles...",
        videoId
      )

      const response = await new Promise<any>((resolve, reject) => {
        chrome.runtime.sendMessage(
          { action: "fetchAutoGeneratedSubtitles", videoId },
          (result) => {
            if (chrome.runtime.lastError) {
              reject(chrome.runtime.lastError)
            } else {
      ...
      if (response && response.vtt) {
        const parsedSubtitles = this.parseVTT(response.vtt)
        this.subtitle1Data = parsedSubtitles
        this.lastProcessedSubtitle1Text = ""
        console.log(
          "[Custom Subtitles] Loaded " +
            parsedSubtitles.length +
            " auto-generated cues"
        )
      } else {
        console.error(
          "[Custom Subtitles] Auto-generated fetch failed:",
          response?.error
        )
      }
    } catch (error) {
      console.error(
        "[Custom Subtitles] Error fetching auto-generated subtitles:",
        error
      )
    }
  }
