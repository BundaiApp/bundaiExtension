...
function MainPage({ onOpenTabs }) {
  ...
  type SubtitleMode = 'api' | 'auto' | 'reader'
  const [subtitleMode, setSubtitleMode] = useState<SubtitleMode>('api')
  ...
  const notifySubtitleModeChange = async (mode: SubtitleMode, useAutoGenerated: boolean, universalReaderEnabled: boolean) => {
    try {
      ...
      chrome.runtime.sendMessage(
        {
          ...
          universalReaderEnabled: universalReaderEnabled
        },
        (response) => {
          if (chrome.runtime.lastError) {
            console.error("Error notifying background:", chrome.runtime.lastError)
          } else {
            console.log("[Popup] Background notified of subtitle mode change:", mode)
          }
        }
      )
    } catch (error) {
  ...
  useEffect(() => {
    ...
    secureStorage.get("subtitleMode").then((value) => {
      ...
      if (isYouTube && value && ['api', 'auto', 'reader'].includes(value)) {
        setSubtitleMode(value as SubtitleMode)
      } else if (!isYouTube) {
        setSubtitleMode('reader')
      } else {
        setSubtitleMode('api')
      }
  ...
  const handleSubtitleModeChange = async (newMode: SubtitleMode) => {
    console.log("[MainPage] subtitle mode changed ->", newMode)
    setSubtitleMode(newMode)

    // Map mode to boolean values for backward compatibility
    const useAutoGenerated = newMode === 'auto'
    const universalReaderEnabled = newMode === 'reader'

    await secureStorage.set("subtitleMode", newMode)
    await secureStorage.set("useAutoGeneratedSubtitles", useAutoGenerated)
    await secureStorage.set("universalReaderEnabled", universalReaderEnabled)
    
    // Notify background script
    await notifySubtitleModeChange(newMode, useAutoGenerated, universalReaderEnabled)
    
    // Show refresh message in popup
    setShowRefreshMessage(true)
  }
  
  const handleWordCardStyleChange = async (styleKey: string, value: string | number) => {
  ...
  return (
    <div className="w-72 p-4 bg-yellow-400 text-black flex flex-col gap-4">
      ...
      {enabled && (
        <div className="bg-white bg-opacity-50 p-3 rounded border-2 border-black">
          <div 
            ...
            <span className="text-xl font-bold">{showStyleEditor ? "▼" : "▶"}</span>
          </div>
          ...
          {showStyleEditor && (
            <div className="mt-3 space-y-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) => handleWordCardStyleChange("backgroundColor", e.target.value)}
                    className="w-12 h-8 rounded cursor-pointer"
                  />
                  <span className="text-xs">{wordCardStyles.backgroundColor}</span>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) => handleWordCardStyleChange("textColor", e.target.value)}
                    className="w-12 h-8 rounded cursor-pointer"
                  />
              ...
              {/* Border Color */}
              <div>
                <label className="text-xs font-semibold block mb-1">Border Color</label>
                <div className="flex items-center gap-2">
                  <input
                    type="color"
                    value={wordCardStyles.borderColor}
                    onChange={(e) => handleWordCardStyleChange("borderColor", e.target.value)}
                    className="w-12 h-8 rounded cursor-pointer"
                  />
              ...
              {/* Word Font Size */}
              <div>
                <label className="text-xs font-semibold block mb-1">Word Size: {wordCardStyles.wordFontSize}px</label>
                <input
                  type="range"
                  min="24"
                  max="72"
                  value={wordCardStyles.wordFontSize}
                  onChange={(e) => handleWordCardStyleChange("wordFontSize", parseInt(e.target.value))}
                  className="w-full"
                />
              </div>
              ...
              {/* Content Font Size */}
              <div>
                <label className="text-xs font-semibold block mb-1">Content Size: {wordCardStyles.fontSize}px</label>
                <input
                  type="range"
                  min="12"
                  max="28"
                  value={wordCardStyles.fontSize}
                  onChange={(e) => handleWordCardStyleChange("fontSize", parseInt(e.target.value))}
                  className="w-full"
                />
              </div>
              ...
              {/* Border Radius */}
              <div>
                <label className="text-xs font-semibold block mb-1">Border Radius: {wordCardStyles.borderRadius}px</label>
                <input
                  type="range"
                  min="0"
                  max="48"
                  value={wordCardStyles.borderRadius}
                  onChange={(e) => handleWordCardStyleChange("borderRadius", parseInt(e.target.value))}
                  className="w-full"
                />
              </div>
      ...
      {enabled && (
        <div className="bg-white bg-opacity-50 p-3 rounded border-2 border-black">
          <h3 className="text-black font-bold mb-2">Mode Selection</h3>
          
          {isYouTubePage ? (
            // YouTube: Show all 3 options
            <div className="flex flex-col gap-2">
              ...
              <label className="flex items-center gap-2 cursor-pointer mt-2">
                <input
                  type="radio"
                  name="subtitleMode"
                  checked={subtitleMode === 'reader'}
                  onChange={() => handleSubtitleModeChange('reader')}
                  className="w-4 h-4"
                />
            ...
            // Non-YouTube: Only Reader Mode (pre-selected, disabled)
            <div className="flex flex-col gap-2">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="radio"
      ...
      {/* Conditional subtitle section - only show for API mode */}
      {enabled && isYouTubePage && currentVideoId && subtitleMode === 'api' ? (
      ...
      ) : enabled && isYouTubePage && currentVideoId && subtitleMode === 'reader' ? (
        <div className="mt-4 p-3 bg-green-100 rounded">
          <h3 className="text-green-700 font-bold">Reader Mode Active</h3>
          <p className="text-xs text-green-600 mt-1">
            Hover over Japanese text to see definitions. Works with any text on the page.
          </p>
        </div>
        ...
        <div className="mt-4 p-3 bg-green-100 rounded">
          <h3 className="text-green-700 font-bold">Reader Mode Active</h3>
          <p className="text-xs text-green-600 mt-1">
            Hover over Japanese text to see definitions. Works on any website.
          </p>
        </div>
      ) : (
