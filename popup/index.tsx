...
function MainPage({ onOpenTabs }) {
  ...
  // New state for subtitle mode (simplified: api or reader)
  type SubtitleMode = 'api' | 'reader'
  ...
  const notifySubtitleModeChange = async (mode: SubtitleMode, universalReaderEnabled: boolean) => {
    try {
  ...
  useEffect(() => {
    ...
    secureStorage.get("subtitleMode").then((value) => {
      ...
      // Migrate 'auto' to 'reader' (auto mode removed)
      const migratedValue = value === 'auto' ? 'reader' : value
      if (isYouTube && migratedValue && ['api', 'reader'].includes(migratedValue)) {
        setSubtitleMode(migratedValue as SubtitleMode)
      } else if (!isYouTube) {
        setSubtitleMode('reader')
      } else {
        setSubtitleMode('api')
      }
      console.log("[MainPage] subtitleMode:", migratedValue)
    })
  ...
  const handleSubtitleModeChange = async (newMode: SubtitleMode) => {
    ...
    await notifySubtitleModeChange(newMode, universalReaderEnabled)
    ...
    <div className="w-72 p-4 bg-yellow-400 text-black flex flex-col gap-4">
      ...
      {enabled && (
        <div className="bg-white bg-opacity-50 p-3 rounded border-2 border-black">
          ...
          {isYouTubePage ? (
            // YouTube: Show API and Reader options
            <div className="flex flex-col gap-2">
              ...
              <p className="text-xs text-gray-600 ml-6">
                Custom subtitle container with dual language support
              </p>
              ...
              <p className="text-xs text-gray-600 ml-6">
                Hover over any Japanese text to see definitions
              </p>
            </div>
          ) : (
            // Non-YouTube: Only Reader Mode (pre-selected)
            <div className="flex flex-col gap-2">
