...
function MainPage({ onOpenTabs }) {
  ...
  // New function to notify content scripts about subtitle mode change
  const notifySubtitleModeChange = async (useAutoGenerated: boolean) => {
    try {
      ...
      if (tab?.id) {
        // Notify custom subtitle container to disable/enable
        chrome.tabs.sendMessage(
          tab.id,
          {
            action: "setExtensionEnabled",
            enabled: !useAutoGenerated // Custom container is disabled when auto-generated is on
          },
          (response) => {
            if (chrome.runtime.lastError) {
              console.log("Custom subtitle container not ready")
            } else {
              console.log("Custom subtitle container state:", !useAutoGenerated)
            }
          }
        )

        // Notify YouTube manipulator to enable/disable
        chrome.tabs.sendMessage(
          tab.id,
          {
          ...
          (response) => {
            if (chrome.runtime.lastError) {
              console.log("YouTube manipulator not ready")
            } else {
              console.log("YouTube manipulator state:", useAutoGenerated)
            }
          }
        )
      }
  ...
  useEffect(() => {
    ...
    secureStorage.get("useAutoGeneratedSubtitles").then((value) => {
      ...
      notifySubtitleModeChange(autoGenValue)
