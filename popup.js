...
document.addEventListener("DOMContentLoaded", function () {
      ...
      chrome.tabs.sendMessage(
        ...
        function (response) {
          if (response && response.tracks) {
            ...
            let firstJapaneseValue = null;
            response.tracks.forEach((track) => {
              if (track.id !== "cc") {
                // Skip CC as we already added it
                const option1 = document.createElement("option");
                option1.value = track.id;
                option1.textContent = track.label;
                // Detect Japanese track by label or languageCode
                const isJapanese =
                  /japanese|日本語|ja[-_]/i.test(track.label) ||
                  (track.languageCode && /ja[-_]/i.test(track.languageCode));
            ...
            // Always select the first Japanese for subtitle1, or CC if none
            if (firstJapaneseValue) {
              subtitle1Select.value = firstJapaneseValue;
            } else {
  ...
  chrome.storage.sync.get(
    ...
    function (items) {
      ...
      document.getElementById("subtitle1-bg").value =
        items.subtitle1.background;
      document.getElementById("subtitle1-opacity").value =
        items.subtitle1.opacity;
      document.getElementById("subtitle1-size").value =
        items.subtitle1.fontSize;

      document.getElementById("subtitle2-color").value = items.subtitle2.color;
      document.getElementById("subtitle2-bg").value =
        items.subtitle2.background;
      document.getElementById("subtitle2-opacity").value =
        items.subtitle2.opacity;
      document.getElementById("subtitle2-size").value =
        items.subtitle2.fontSize;
      ...
      document.getElementById("card-opacity").value =
        items.wordCard.backgroundOpacity;
      document.getElementById("card-text-color").value =
        items.wordCard.textColor;
      document.getElementById("card-border-radius").value =
        items.wordCard.borderRadius;
      document.getElementById("card-padding").value = items.wordCard.padding;
      document.getElementById("card-shadow").value =
        items.wordCard.shadowIntensity;
  ...
  document
    .getElementById("toggle-subtitles")
