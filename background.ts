...
async function initializeState() {
  ...
  try {
    ...
    extensionState.wordCardStyles =
      ...
      !Array.isArray(stylesValue)
        ? {
            ...defaultWordCardStyles,
            ...(stylesValue as Partial<WordCardStyles>)
          }
        : { ...defaultWordCardStyles }
    ...
    extensionState.subtitleContainerStyles =
      subtitleStylesValue &&
      typeof subtitleStylesValue === "object" &&
      !Array.isArray(subtitleStylesValue)
        ? {
            ...defaultSubtitleContainerStyles,
            ...(subtitleStylesValue as Partial<SubtitleContainerStyles>)
          }
        : { ...defaultSubtitleContainerStyles }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  // Legacy support for useAutoGeneratedSubtitles
  // Returns FALSE when in API mode (custom container enabled)
  // Returns TRUE when in USER or ASR mode (custom container disabled)
  if (message.action === "getUseAutoGeneratedSubtitles") {
    ensureStateInitialized().then(() => {
      sendResponse({ useAutoGenerated: extensionState.subtitleMode !== "api" })
    })
    return true
  }
  ...
  if (message.action === "setSubtitleContainerStyles") {
    ensureStateInitialized().then(() => {
      ...
      storage
        .set("subtitleContainerStyles", extensionState.subtitleContainerStyles)
        .catch(console.error)
