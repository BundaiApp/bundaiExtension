...
async function broadcastStateToAllTabs() {
  try {
    ...
    for (const tab of allTabs) {
      ...
      if (isYouTube) {
        // YouTube tabs: handle all three scripts
        chrome.tabs.sendMessage(tab.id, {
          action: "setExtensionEnabled",
          enabled: extensionState.extensionEnabled && !extensionState.useAutoGeneratedSubtitles && !extensionState.universalReaderEnabled
        }).catch(() => {
      ...
      // All tabs: universal reader
      chrome.tabs.sendMessage(tab.id, {
        action: "setUniversalReaderEnabled",
        enabled: extensionState.extensionEnabled && extensionState.universalReaderEnabled
      }).catch(() => {
        // Content script might not be loaded yet, ignore
      })
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === "setSubtitleMode") {
    ensureStateInitialized().then(() => {
      const { mode, useAutoGenerated, universalReaderEnabled } = message
      
      extensionState.useAutoGeneratedSubtitles = useAutoGenerated
      extensionState.universalReaderEnabled = universalReaderEnabled
      
      // Persist to storage
      storage.set("useAutoGeneratedSubtitles", useAutoGenerated).catch(console.error)
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (changeInfo.status === 'complete' && tab.url) {
    ...
    console.log("[Background] Tab loaded, broadcasting state. YouTube:", isYouTube)
