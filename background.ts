...
async function initializeState() {
  ...
  try {
    ...
    extensionState.useAutoGeneratedSubtitles =
      typeof autoGenValue === "boolean" ? autoGenValue : false

    // Load subtitleMode
    const modeValue = await storage.get("subtitleMode")
    extensionState.subtitleMode =
      modeValue && ["api", "auto", "user"].includes(modeValue as string)
        ? (modeValue as "api" | "auto" | "user")
        : "user"
...
async function broadcastStateToAllTabs() {
  try {
    ...
    for (const tab of tabs) {
      ...
      // Send to YouTube manipulator (no longer used, keeping for compatibility)
      chrome.tabs
        .sendMessage(tab.id, {
...
async function broadcastSubtitleModeToAllTabs(mode: "api" | "auto" | "user") {
  try {
    const tabs = await chrome.tabs.query({})
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === "getUseAutoGeneratedSubtitles") {
    ensureStateInitialized().then(() => {
      console.log(
        "[Background] Responding to getUseAutoGeneratedSubtitles:",
        extensionState
      )
      sendResponse({
  ...
  if (message.action === "setUseAutoGeneratedSubtitles") {
    ensureStateInitialized().then(() => {
      ...
      // Persist to storage
      storage
        .set("useAutoGeneratedSubtitles", message.useAutoGenerated)
        .catch(console.error)
  ...
  if (message.action === "setSubtitleMode") {
    ensureStateInitialized().then(() => {
      const mode = message.subtitleMode as "api" | "auto" | "user"
      if (["api", "auto", "user"].includes(mode)) {
  ...
  // Subtitle container styles state
...
// Fetch auto-generated subtitles via GraphQL
async function handleFetchAutoGeneratedSubtitles(message, sendResponse) {
  try {
    ...
    console.log(
      `[Background] Fetching auto-generated subtitles for video: ${videoId}`
    )
    ...
    const transcript = data.data.getAutoTranscript

    if (transcript.success && transcript.vtt) {
      sendResponse({ success: true, vtt: transcript.vtt })
      console.log(
        `[Background] Successfully fetched ${transcript.segmentCount} segments`
      )
    } else {
      throw new Error(transcript.error || "Failed to fetch transcript")
    }
  } catch (error) {
    console.error(
      `[Background] Error fetching auto-generated subtitles: ${error.message}`
    )
    sendResponse({
      success: false,
...
// Add handler for fetchAutoGeneratedSubtitles action
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === "fetchAutoGeneratedSubtitles") {
    handleFetchAutoGeneratedSubtitles(message, sendResponse)
    return true
  }
})
