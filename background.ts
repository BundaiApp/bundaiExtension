...
async function broadcastStateToAllTabs() {
  try {
    ...
    // Determine mode: 'api' when neither auto nor reader is enabled
    const isApiMode = !extensionState.useAutoGeneratedSubtitles && !extensionState.universalReaderEnabled
    // Reader handles both 'auto' and 'reader' modes now
    const isReaderMode = extensionState.useAutoGeneratedSubtitles || extensionState.universalReaderEnabled
    ...
    for (const tab of allTabs) {
      ...
      // All tabs: universal reader (handles both 'auto' and 'reader' modes)
      chrome.tabs.sendMessage(tab.id, {
